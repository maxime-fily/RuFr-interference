...
print(str(args.work_file).split("_")[1].split("-")[0])
LAYPFX=f"L{str(args.work_file).split('_')[1].split('-')[0]}"
...
#    ax3 = fig.add_subplot(211)
    ...
    ax3.set_title("Vector cost Matrix ; alignment cost={:.4f}".format(cost_mat[-1, -1]))
    ax3.set_xlim([-0.5, lcm.T.shape[1]-0.5])
    ax3.set_ylim([-0.5, lcm.T.shape[0]-0.5])
    xb = np.arange(b.shape[1])
    yb = np.arange(b.shape[0])
    Xb, Yb = np.meshgrid(xb, yb)
#horizontal stacking
...
#    ax4 = fig.add_subplot(212)
    ax4.quiver(Yb, Xb, grad_col, grad_row, color='white', scale=5)
    ...
    ax4.set_xlim([-0.5, lcm.T.shape[1]-0.5])
    ax4.set_ylim([-0.5, lcm.T.shape[0]-0.5])
    ax4.set_xlabel(xname)
    ax4.set_ylabel(yname)
    fig.figure.savefig(f"{LAYPFX}_{args.fich_x}-{args.locut_x}_vs_{args.fich_y}-{args.locut_y}_2DTW.pdf", bbox_inches='tight')
