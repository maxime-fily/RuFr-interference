...
#    ax4 = fig.add_subplot(212)
    ...
    fig = plt.figure()
    fig = plt.imshow(b.T, origin='lower', cmap='twilight', interpolation='nearest', vmin=0.2, vmax=1.)
    plt.plot(a.index1, a.index2, color='red', marker='x', markersize=3, label="vect. DTW")
    plt.plot(scaled_path_x, scaled_path_y, color='green', marker='o', markersize=3, label="Audio DTW")
    plt.title(f"Layer_{str(args.work_file).split('_')[1].split('-')[0]}")
    plt.xlim([-0.5, lcm.T.shape[1]-0.5])
    plt.ylim([-0.5, lcm.T.shape[0]-0.5])
    plt.xlabel(xname)
    plt.ylabel(yname)
    fig.figure.savefig(f"Layer_{str(args.work_file).split('_')[1].split('-')[0]}_{str(args.work_file).split('_')[1].split('-')[2].replace('.pkl','')}_{args.fich_x}-{args.locut_x}_vs_{args.fich_y}-{args.locut_y}_cosine_sim.pdf", bbox_inches='tight')
    #print(str(lcm.T.shape[0]),str(lcm.T.shape[1]))
#    plt.show()
#    plt.close()
