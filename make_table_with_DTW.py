...
#    print("en y : ", scaled_path_y)
    ...
    listR.append([couple['DTW'],couple['speaker_x'], couple['filename_x'], couple['speaker_y'], couple['filename_y'], a.index1, a.index2, mini, maxi, len(a.index1), \
                  (len(a.index1)-mini)/(maxi-mini), listD1, listD2, a1, a2, a.distance, a.normalizedDistance, maxinrow, np.max(abs_row), maxincol, np.max(abs_col), list3m, scaled_path, path, cost_mat])
...
dR.columns = ["DTW","spk_x", "file_x", "spk_y", "file_y", "index1", "index2", "min", "max", "vect_path_len", "R", 'dev_x', 'dev_y', 'm', 'n', 'dist_A', 'dist_N','MrowGpos','MrowG','McolGpos','McolG','Cdistinfo','scaled_audio_path',"audio_path", "audio_cost_mat"]
...
argout = str(args.work_file).replace("allspk-","")
output_name = f"table_{str(argout)}"
