...
parser.add_argument("--fich_x", required=True, type=Path)
parser.add_argument("--locut_x", required=True, type=Path)
parser.add_argument("--fich_y", required=True, type=Path)
parser.add_argument("--locut_y", required=True, type=Path)
...
print(df[['filename_x','filename_y', 'speaker_x', 'speaker_y']])
df = df[(df['filename_x']==str(args.fich_x))&(df['speaker_x']==str(args.locut_x))&(df['filename_y']==str(args.fich_y))&(df['speaker_y']==str(args.locut_y))]
print(df)
...
#Audio DTW plot
    ...
    fig = plt.figure(figsize=(lcm.T.shape[1], lcm.T.shape[0]))
...
img = ax.imshow(b.T, origin='lower', cmap='twilight', interpolation='nearest', vmin=0.2, vmax=1.)  # Use a specific variable for imshow output

# Plotting additional data
ax.plot(a.index1, a.index2, color='red', marker='x', markersize=3, label="vect. DTW")
ax.plot(scaled_path_x, scaled_path_y, color='green', marker='o', markersize=3, label="Audio DTW")

# Adding title, labels, and limits
ax.set_title(f"Layer_{str(args.work_file).split('_')[1].split('-')[0]}")
ax.set_xlim([-0.5, lcm.T.shape[1] - 0.5])
ax.set_ylim([-0.5, lcm.T.shape[0] - 0.5])
ax.set_xlabel(xname)
ax.set_ylabel(yname)
...
# Save the figure
print(str(args.work_file).split('_')[1].split('-')[2].replace(".pkl",""))
fig.savefig(f"Layer_{str(args.work_file).split('_')[1].split('-')[0]}_{str(args.work_file).split('_')[1].split('-')[2].replace('.pkl','')}_{args.fich_x}-{args.locut_x}_vs_{args.fich_y}-{args.locut_y}_cosine_sim.pdf", bbox_inches='tight')    #print(str(lcm.T.shape[0]),str(lcm.T.shape[1]))
#    plt.show()
#    plt.close()
