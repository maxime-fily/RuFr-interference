...
print("pre normalization row count : ")
print(df.shape[0])
...
# whatsoever shall be done with it.

print("normalization...")
for ID in din:
    speaker_df = df[df["speaker"] == ID]
    print("speaker " + ID + "...")
    normalized_col = f"normalized_{args.repr_column}"
    X = np.vstack(speaker_df[args.repr_column].values)
    mu = X.mean(axis=0)
    sigma = X.std(axis=0)
    df.loc[df["speaker"] == ID, normalized_col] = speaker_df[args.repr_column].apply(lambda x: (x - mu) / sigma)

print("post normalization row count : ")
print(df.shape[0])
